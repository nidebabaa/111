...
#include "Quantize.h"
#include "ConBlur.h"
#include "ConMotionBlur.h"
#include "ConEdge.h"
#include "ConSharpen.h"
...
void FlashPhotoApp::ApplyBlurFilter(float radius) {
  SaveStateForPossibleUndo();
  (void)radius;
  ConBlur myfilter = ConBlur(radius);
  myfilter.ApplyToBuffer(pixel_buffer());
}
...
void FlashPhotoApp::ApplyMotionBlurFilter(
  ...
  (void)rad;
  (void)dir;
  ConMotionBlur myfilter = ConMotionBlur(rad,dir);
  myfilter.ApplyToBuffer(pixel_buffer());
}
...
void FlashPhotoApp::ApplySharpenFilter(float rad) {
  SaveStateForPossibleUndo();
  (void)rad;
  ConSharpen myfilter = ConSharpen(radius);
  myfilter.ApplyToBuffer(pixel_buffer());
}
...
void FlashPhotoApp::ApplyThresholdFilter(float value) {
  SaveStateForPossibleUndo();
  (void)value;
  Threshold myFilter = Threshold(value);
  myFilter.ApplyToBuffer(pixel_buffer());
}
...
void FlashPhotoApp::ApplySaturateFilter(float scale) {
  SaveStateForPossibleUndo();
  (void)scale;
  Saturate myFilter = Saturate(scale);
  myFilter.ApplyToBuffer(pixel_buffer());
}
...
void FlashPhotoApp::ApplyChannelsFilter(float red, float green, float blue) {
  ...
  (void)green;
  (void)blue;
  Channels myFilter = Channels(red,green,blue);
  myFilter.ApplyToBuffer(pixel_buffer());
...
void FlashPhotoApp::ApplyQuantizeFilter(int num) {
  SaveStateForPossibleUndo();
  (void)num;
  Quantize myFilter = Quantize(num);
  myFilter.ApplyToBuffer(pixel_buffer());
}
